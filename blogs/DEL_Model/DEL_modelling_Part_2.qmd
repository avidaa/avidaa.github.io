...
```{python}
#| label: fig-umbrella-raw-heatmap
#| fig-cap: "Umbrella use and getting wet in the simulated population. Tiles show counts and percentages of all observations."
...
```{python}
#| label: fig-umbrella-weather-heatmap
#| fig-cap: "Umbrella use and getting wet after stratifying by weather. Percentages are calculated within each weather group."
...
```{python}
#| label: fig-umbrella-bias-sensitivity
#| fig-cap: "Sensitivity of the inferred umbrella effect to an assumed hidden-weather bias. The curve and band show the posterior median and central 80% interval after bias correction."
...
p_sensitivity = (
    ...
    + labs(
        ...
        subtitle=(
            ...
            " (see the hidden-weather DAG above).\n"
...
```{python}
#| label: fig-latent-weather-posteriors
#| fig-cap: "Posterior distributions of the path coefficients in the model with unobserved weather."
...
```{python}
#| label: fig-latent-weather-prior-posterior
#| fig-cap: "Prior and posterior distributions of the latent-weather model coefficients, showing how the observations update the assumed effects."
...
```{python}
#| label: fig-pipe-simulated-associations
#| fig-cap: "Simulated mediation example. The upper panel compares weight loss by exercise status; the lower panel relates weight loss to blood pressure."
...
```{python}
#| label: fig-pipe-effect-posteriors
#| fig-cap: "Posterior distributions for the total exercise effect, the direct exercise effect conditional on weight loss, and the weight-loss coefficient conditional on exercise."
...
```{python}
#| label: fig-collider-full-population
#| fig-cap: "Smart and Athletic in the full simulated population, before conditioning on admission. The line shows the fitted linear association."
...
```{python}
#| label: fig-collider-admitted-population
#| fig-cap: "Smart and Athletic among admitted applicants. Selecting on admission induces an association between the two traits."
...
```{python}
#| label: fig-collider-coefficient-posteriors
#| fig-cap: "Posterior distributions of the Smart coefficient when modelling Athletic, with and without conditioning on the admission collider."
...
```{python}
#| label: fig-collider-descendant-associations
#| fig-cap: "Associations between Smart and Athletic in the full population, among admitted applicants, and among applicants included in the student database, a descendant of admission."
...
```{python}
#| label: fig-collider-descendant-posteriors
#| fig-cap: "Posterior distributions of the Smart coefficient with no adjustment, adjustment for admission, or adjustment for student-database membership."
...
When the expected count is very small, most observations are zero even without overdispersion. For example, at $\mu=0.1$, the probability of zero is approximately 90.48% under Poisson sampling and 90.53% at the prior median of $\kappa_c$. This makes the effect of dispersion less visible than in the larger-count examples. An expected count of $0.1$ is an average across hypothetical repetitions; individual observed counts are still integers.

Each condition has its own unknown $\kappa_c$, shared across its compounds and replicates. Using the same prior gives library, matrix, and target conditions the same starting assumptions, while allowing their fitted dispersion values to differ. The choice of $0.5$ is provisional: a smaller prior spread would favour less overdispersion, while a larger spread would allow more. Holding $\mu$ fixed in these plots isolates this choice. A prior predictive check of the complete model also draws the intercepts and latent effects to obtain each observation's $\mu$, then checks whether the resulting counts have plausible zero frequencies and upper tails.
