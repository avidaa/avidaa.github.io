...
which sum to zero. The common value $0.3$ is absorbed by the condition intercepts ($\alpha_c$, $c\in\{l,m,t\}$), while the final [$a_i$]{style="color:#a3be8c"} values describe compound-specific deviations from that common level.

In the non-centred implementation, $a_i^*=\sigma_a z_{a,i}$ and $\bar a^*=\sigma_a\bar z_a$. Therefore,

TODO: 
1. explain what does "The common value $0.3$ is absorbed by the condition intercepts ($\alpha_c$, $c\in\{l,m,t\}$), while the final [$a_i$]{style="color:#a3be8c"} values describe compound-specific deviations from that common level." mean?

2. We can break this into three sections, hierarchical prior, zero-sum transformation, and non-centered version.
...
which is the zero-sum transformation used in the model. The same calculation is applied separately to [$b_i$]{style="color:#88c0d0"}, [$s_i$]{style="color:#bf616a"}, $\gamma_{m,r}$, and $\gamma_{t,r}$.
