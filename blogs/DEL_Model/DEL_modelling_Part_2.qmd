...
$$
p\left(\textcolor{#bf616a}{s_i} \mid \mathbf{C}_l,\mathbf{C}_m,\mathbf{C}_t\right),
$$
...
![Observed counts for one compound.](img/observed_counts_only.png){#fig-dag-ct-only width="50%"}
...
![DAG showing input abundance for one compound.](img/dag_with_input_abundance.png){#fig-dag-with-input-abundance width="50%"}
...
![DAG showing matrix background for one compound.](img/dag_with_matrix_background.png){#fig-dag-with-matrix-background width="50%"}
...
![DAG showing target-specific enrichment for one compound.](img/dag_with_target_specific_enrichment.png){#fig-dag-with-target-specific-enrichment width="50%"}
...
![DAG showing observed counts for multiple compounds.](img/dag_with_plate.png){#fig-dag-with-plate width="55%"}
...
![DAG showing observed counts, input abundance, matrix background, target-specific enrichment, and replicate effects.](img/dag_with_latent_factors_and_replicates.png){#fig-dag-with-latent-factors-and-replicates width="50%"}
...
![Path from $\textcolor{#bf616a}{s_i}$ to $C_{t,i,r}$.](img/path_s_to_c_t.png){#fig-path-s_i-to-Ct_i_r width="50%"}
...
![Path from $\textcolor{#bf616a}{s_i}$ to $C_{l,i}$.](img/path_s_to_c_l.png){#fig-path-sto-Cl width="50%"}
...
![Posterior distributions for the umbrella example.](img/fork_weather_example_posteriors.png){#fig-fork-weather-example-posteriors width="50%"}
...
![Hidden weather confounder DAG for sensitivity analysis](img/sensitivity_analysis_DAG.png){#fig-sensitivity-analysis-DAG width="50%"}
...
![One-sided sensitivity analysis: higher assumed hidden bias shifts the corrected effect to more negative values.](img/sensitivity_analysis_explainer.png){#fig-sensitivity-analysis-explainer width="50%"}
...
![Sensitivity analysis for umbrella example](img/sensitivity_analysis_for_umbrella_example.png){#fig-sensitivity-analysis-for-umbrella-example width="50%"}
...
![Collider example DAG.](img/collider_example.png){#fig-collider-example width="50%"}
...
![Descendant example DAG.](img/descendant_example.png){#fig-descendant-example width="50%"}
...
- **Credible-interval coverage:** for each simulation, record whether the credible interval contains the known generating value. For example, the coverage of a 90% interval is
  ...
  Substantially **lower** coverage suggests intervals that are too narrow or estimates that are biased.
  Substantially **higher** coverage may indicate unnecessarily wide intervals. 
...
All additive effects ([$\textcolor{#a3be8c}{a_i}$]{style="color:#a3be8c"}, [$b_i$]{style="color:#88c0d0"}, [$s_i$]{style="color:#bf616a"}, $\gamma_{m,r}$, and $\gamma_{t,r}$) live on the log-mean scale.
They are “additive” because they are summed on the log scale, for example:
...
Writing $E_c$ generically for the relevant known exposure offset ($E_l$, $E_{m,r}$, or $E_{t,r}$), the baseline expected count when the compound and replicate effects are zero is:
$$
...
The $z$ variables are standardised deviations without direct scientific units. Subtracting their means enforces the zero-sum constraints, and multiplying by the corresponding $\sigma$ gives each effect its population scale. Because all compounds share an estimated population scale, noisy compound-level effects are partially pooled toward zero, while strongly supported effects can remain far from zero. This is the hierarchical shrinkage discussed in the parameter-recovery section.

:::: {.callout-note collapse="true" icon="false"}
## Why use this parameterisation?

This [non-centred hierarchical parameterisation](https://betanalpha.github.io/assets/case_studies/hierarchical_modeling.html) constructs the latent-effect parameters, [$a_i$]{style="color:#a3be8c"}, [$b_i$]{style="color:#88c0d0"}, [$s_i$]{style="color:#bf616a"}, $\gamma_{m,r}$, and $\gamma_{t,r}$, from unit-scale auxiliary variables $z \sim \operatorname{Normal}(0,1)$ and the population-scale parameters $\sigma_a$, $\sigma_b$, $\sigma_s$, $\sigma_{\gamma_m}$, and $\sigma_{\gamma_t}$, which are shared across all compounds and replicates and discussed in [Population scales](#population-scales).


Consider the input-abundance effects [$a$]{style="color:#a3be8c"} as an example. The model contains one effect for every compound, [$a_i$]{style="color:#a3be8c"}. Estimating them independently would produce unstable values for compounds with few reads. We therefore use a common population scale, $\sigma_a$, whose spread is learned from all compounds, then given $\sigma_a$, the [$a_i^*$]{style="color:#a3be8c"} effect is drawn from a Normal distribution with mean 0 and variance $\sigma_a^2$:
...
\textcolor{#a3be8c}{a_i^*}\mid\sigma_a
    &\sim\operatorname{Normal}(0,\sigma_a^2),
    &&\text{compound level}.
\end{aligned}
$$

This conditional dependence is what makes the prior **hierarchical**: the compound-level effects [$a_i^*$]{style="color:#a3be8c"} share the higher-level parameter $\sigma_a$. Data from all compounds inform $\sigma_a$, and the inferred $\sigma_a$ in turn controls how far the abundance effects can plausibly lie from zero. This sharing produces **partial pooling**: uncertain effects are regularised toward zero, while compounds with strong evidence can retain larger effects. We wrote [$a_i^*$]{style="color:#a3be8c"} to emphasise that it is a preliminary effect, not the final [$a_i$]{style="color:#a3be8c"}. The final effect satisfies the zero-sum constraint discussed in [Are the model parameters identifibale?](#are-the-model-parameters-identifiable){.box-ref-highlight}.

The connection to the standardised variable $z_{a,i}$ follows from a basic property of the Normal distribution. Draw

$$
z_{a,i}\sim\operatorname{Normal}(0,1)
$$

and define the preliminary effect as

$$
\textcolor{#a3be8c}{a_i^*}=\sigma_a z_{a,i}.
$$

Conditional on $\sigma_a$, multiplying a standard Normal variable by $\sigma_a$ gives

$$
\textcolor{#a3be8c}{a_i^*}\mid\sigma_a
\sim\operatorname{Normal}(0,\sigma_a),
$$

which is exactly the compound-level line in the hierarchy above. The preliminary effects $a_i^*$ do not yet satisfy the zero-sum constraint, so we centre them:

$$
\begin{aligned}
\textcolor{#a3be8c}{a_i}
&=\textcolor{#a3be8c}{a_i^*}-\textcolor{#a3be8c}{\bar a^*}\\
&=\sigma_a z_{a,i}-\sigma_a\bar z_a\\
&=\sigma_a\left(z_{a,i}-\bar z_a\right).
\end{aligned}
$$

Thus, the final [$a_i$]{style="color:#a3be8c"} is the zero-sum version of the unconstrained hierarchical effect $a_i^*$. The same construction is used for [$b_i$]{style="color:#88c0d0"}, [$s_i$]{style="color:#bf616a"}, $\gamma_{m,r}$, and $\gamma_{t,r}$, each with its corresponding population-scale parameter.

**Why subtract the group mean?** This makes the latent effects identifiable separately from the condition intercepts. Suppose we add a constant $d$ to every [$a_i$]{style="color:#a3be8c"} and subtract $d$ from $\alpha_l$, $\alpha_m$, and $\alpha_t$. Every linear predictor—and therefore every expected count—remains unchanged. The likelihood cannot determine whether this common shift belongs to the intercepts or to [$a_i$]{style="color:#a3be8c"}. Corresponding ambiguities occur between the other groups of effects and their condition intercepts, as explained in [Are the model parameters identifiable?](#are-the-model-parameters-identifiable){.box-ref-highlight}.

Subtracting the mean of the $z$ values removes this ambiguity. For example, because

$$
\bar z_a=\frac{1}{N}\sum_{i=1}^{N}z_{a,i},
$$

the transformed abundance effects satisfy

$$
\sum_{i=1}^{N}\textcolor{#a3be8c}{a_i}
=
\sigma_a\left(
\sum_{i=1}^{N}z_{a,i}-N\bar z_a
\right)
=0.
$$

The other transformations enforce the corresponding zero-sum constraints. Zero is an **identification convention**, not a claim that the average biological effect is literally zero. It assigns each group-wide average to the relevant intercept $\alpha_c$, while the compound and replicate effects describe deviations around that average.

**Why multiply by $\sigma$?** After centring, the $z$ values still have a fixed unit scale. Multiplying them by the relevant population scale gives the effects the amount of variation learned through the hierarchy. For example, a small $\sigma_a$ keeps the [$a_i$]{style="color:#a3be8c"} values close to zero, whereas a larger $\sigma_a$ permits greater differences among compounds.

Separating the unit-scale variable $z$ from the estimated scale $\sigma$ is called a **non-centred parameterisation**. It represents the same hierarchical model described above, but often produces more stable Bayesian computation when $\sigma$ is small or weakly identified.
::::

###### Condition-specific dispersion

Rather than place a prior directly on the NB2 inverse-dispersion parameter $\phi_c$, we define
...
where $\kappa_c$ is the asymptotic extra-Poisson coefficient of variation in condition $c$. For large expected counts, the NB2 coefficient of variation approaches $\kappa_c$. We assign independent, identically specified priors to the three conditions:
...
Values near $\kappa_c=0$ imply very large $\phi_c$ and behaviour close to Poisson. In terms of counts,
...
For example, when $\mu=100$:

- $\kappa_c=0$ gives the Poisson standard deviation $\sqrt{100}=10$ reads.
- The prior median is approximately $0.34$, corresponding to $\phi_c\approx8.7$ and a standard deviation of approximately $\sqrt{100+100^2(0.34)^2}\approx35$ reads.
- $\kappa_c=0.5$ corresponds to $\phi_c=4$ and a standard deviation of approximately $51$ reads.
- $\kappa_c=1$ corresponds to $\phi_c=1$ and a standard deviation of approximately $100$ reads.

Thus, the prior allows count distributions ranging from nearly Poisson to strongly overdispersed. Expressing the prior through $\kappa_c$ makes these count-scale implications easier to understand than placing a diffuse prior directly on $\phi_c$.

###### Checking and revising these assumptions

These numerical scales are modelling assumptions, not facts learned before seeing the counts. Prior predictive simulation should determine whether they generate plausible zero frequencies, upper tails, replicate correlations, and target-to-matrix fold changes. Implausible simulations indicate that the priors should be revised before fitting the observed data.

Sensitivity analysis should also examine the structural assumptions that background contributes to matrix and target counts through the same [$b_i$]{style="color:#88c0d0"}, counts are conditionally independent given the latent effects, and one dispersion parameter is shared by all compounds within each condition. More flexible alternatives include estimating a matrix-to-target loading, assigning a sparse or heavy-tailed prior to [$s_i$]{style="color:#bf616a"}, or replacing NB2 with a Poisson-lognormal likelihood. These extensions should be motivated by failed predictive checks rather than added automatically.
