...
The term $1/\mu$ is the contribution of Poisson counting noise to the **squared CV**, while $\kappa_c^2$ is the contribution of overdispersion. As $\mu$ increases, $1/\mu$ becomes smaller, so the total CV approaches $\kappa_c$. Thus, when Poisson noise is small, $\kappa_c=0.5$ means that the standard deviation is approximately one-half of the expected count.

The transformation from $\phi_c$ to $\kappa_c$ does not change the likelihood or solve an identifiability problem. The CV calculation shows why it makes the prior easier to specify: $\kappa_c$ directly describes the approximate proportional spread of large counts.

For example, if the expected count is $1{,}000$ reads:

- if $\kappa_c=0.5$,
  $\operatorname{SD}=\sqrt{1000+1000^2(0.5)^2}=\sqrt{251{,}000}\approx501$ reads, or about $1{,}000\times0.5=500$ reads using the large-count approximation $\operatorname{SD}\approx\mu\kappa_c$;
- if $\kappa_c=0.1$,
  $\operatorname{SD}=\sqrt{1000+1000^2(0.1)^2}=\sqrt{11{,}000}\approx105$ reads, or about $1{,}000\times0.1=100$ reads.

At smaller expected counts, the Poisson term also contributes noticeably, so the total standard deviation is larger than this simple approximation. A Half-Normal prior on $\kappa_c$ places substantial prior probability near the Poisson model while retaining a right tail that allows stronger overdispersion.
