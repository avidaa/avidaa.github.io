...
To make the hierarchy concrete, consider the input-abundance effects [$a_i$]{style="color:#a3be8c"} as an example. The model contains one effect for every compound. Estimating them independently would produce unstable values for compounds with few reads. We therefore begin with preliminary, unconstrained effects $a_i^*$ that belong to a common population whose spread is learned from all compounds:
