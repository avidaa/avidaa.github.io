...
To interpret $\kappa_c$, compare the **total standard deviation** , $\operatorname{SD}(C\mid\mu,\kappa_c)$, with the mean, $\mathbb E(C\mid\mu,\kappa_c)$. This ratio is the coefficient of variation:
...
In the final expression, $1/\mu$ is the relative contribution from Poisson counting noise and $\kappa_c^2$ is the relative contribution from overdispersion. As $\mu$ increases, $1/\mu$ becomes smaller, so the total coefficient of variation approaches $\kappa_c$.

The transformation from $\phi_c$ to $\kappa_c$ does not change the likelihood or solve an identifiability problem. It makes the prior easier to specify and check: values of $\kappa_c$ can be read directly as the approximate total standard deviation relative to the mean when expected counts are large. A Half-Normal prior on $\kappa_c$ places substantial prior probability near the Poisson model while retaining a right tail that allows stronger overdispersion.
