...
The $z$ variables are standardised deviations without direct scientific units. Subtracting their means enforces the zero-sum constraints, and multiplying by the corresponding $\sigma$ gives each effect its population scale. The box below explains why this construction is useful.
...
This conditional dependence is what makes the prior **hierarchical**: the compound-level effects [$a_i^*$]{style="color:#a3be8c"} share the higher-level parameter $\sigma_a$. Data from all compounds inform $\sigma_a$, and the inferred $\sigma_a$ in turn controls how far the abundance effects can plausibly lie from zero. This sharing produces **partial pooling**: uncertain effects are regularised toward zero, while compounds with strong evidence can retain larger effects. Hierarchical modelling specifies the shared structure; partial pooling is the resulting behaviour. We write [$a_i^*$]{style="color:#a3be8c"} to emphasise that it is a preliminary effect, not the final [$a_i$]{style="color:#a3be8c"}. The final effect satisfies the zero-sum constraint discussed in [Are the model parameters identifiable?](#are-the-model-parameters-identifiable){.box-ref-highlight}.

**Connection to the non-centred form.** The hierarchy above can be expressed using a standardised variable $z_{a,i}$. Draw
...
Multiplication by $\sigma_a$ applies the population scale learned through the hierarchy: a small $\sigma_a$ keeps the abundance effects close to zero, whereas a larger $\sigma_a$ permits greater differences among compounds. Conditional on $\sigma_a$, scaling a standard Normal variable in this way gives
...
**Why subtract the group mean?** This additional step makes the latent effects identifiable separately from the condition intercepts. Suppose we add a constant $d$ to every [$a_i$]{style="color:#a3be8c"} and subtract $d$ from $\alpha_l$, $\alpha_m$, and $\alpha_t$. Every linear predictor—and therefore every expected count—remains unchanged. The likelihood cannot determine whether this common shift belongs to the intercepts or to [$a_i$]{style="color:#a3be8c"}. Corresponding ambiguities occur between the other groups of effects and their condition intercepts, as explained in [Are the model parameters identifiable?](#are-the-model-parameters-identifiable){.box-ref-highlight}.
