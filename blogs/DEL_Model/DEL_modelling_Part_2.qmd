...
###### Population scales for compound and replicate variation
...
This parameterisation constructs the latent-effect parameters, [$a_i$]{style="color:#a3be8c"}, [$b_i$]{style="color:#88c0d0"}, [$s_i$]{style="color:#bf616a"}, $\gamma_{m,r}$, and $\gamma_{t,r}$, from unit-scale auxiliary variables $z \sim \operatorname{Normal}(0,1)$ and the population-scale parameters $\sigma_a$, $\sigma_b$, $\sigma_s$, $\sigma_{\gamma_m}$, and $\sigma_{\gamma_t}$. 

It solves two modelling problems that arise from the hierarchical structure of the model: 

1. Separating each condition intercept ($\alpha_l$, $\alpha_m$, or $\alpha_t$) from **any group-wide shift in the latent effects**, and 
2. Learning how much input-abundance effects [$a_i$]{style="color:#a3be8c"}, matrix-background effects [$b_i$]{style="color:#88c0d0"}, and target-enrichment effects [$s_i$]{style="color:#bf616a"} vary across compounds, and how much $\gamma_{m,r}$ and $\gamma_{t,r}$ vary across matrix and target replicates.

Here, a **group-wide shift** means the arithmetic mean of one type of latent effect: for example, $\bar a=\frac{1}{N}\sum_i a_i$ across compounds or $\bar\gamma_m=\frac{1}{R_m}\sum_r\gamma_{m,r}$ across matrix replicates. 

[Without a constraint, the intercept (e.g. $\alpha_l$) and this common average (e.g. $\bar a$) are indistinguishable because moving a constant from one to the other leaves the expected counts unchanged. The zero-sum transformations set each such average to zero, so the intercept represents the group baseline and the latent effects represent deviations from it.]{.highlight-key-point}

Conceptually, we want hierarchical priors such as
...
with analogous priors for [$b_i$]{style="color:#88c0d0"}, [$s_i$]{style="color:#bf616a"}, $\gamma_{m,r}$, and $\gamma_{t,r}$. A direct implementation, however, would still need the zero-sum constraints.

**Problem 1: additive ambiguity.** Suppose we add a constant $d$ to every [$a_i$]{style="color:#a3be8c"} and subtract $d$ from $\alpha_l$, $\alpha_m$, and $\alpha_t$. Every linear predictor—and therefore every expected count—remains unchanged. The likelihood cannot determine whether this common shift belongs to the intercepts or to [$a_i$]{style="color:#a3be8c"}. Corresponding ambiguities occur between the other groups of effects and their condition intercepts, as explained in [Are the model parameters identifiable?](#are-the-model-parameters-identifiable){.box-ref-highlight}.

Subtracting the group mean removes this ambiguity. For example, because
...
**Problem 2: unstable estimates for weakly observed effects.** Multiplication by $\sigma$ gives each group a shared, data-informed scale. Effects supported by little information are pulled toward the group average of zero, whereas strongly supported effects can remain farther away. This **hierarchical shrinkage** reduces unstable extreme estimates without forcing all compounds or replicates to have the same effect.

Writing each effect as a unit Normal variable $z$ multiplied by an estimated scale $\sigma$ is called a **non-centred parameterisation**. It expresses the same hierarchical idea as a direct conditional Normal prior, but often produces more stable Bayesian computation when $\sigma$ is small or weakly identified.
::::
