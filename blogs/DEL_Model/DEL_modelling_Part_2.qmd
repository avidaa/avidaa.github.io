...
This parameterisation constructs the latent-effect parameters, [$a_i$]{style="color:#a3be8c"}, [$b_i$]{style="color:#88c0d0"}, [$s_i$]{style="color:#bf616a"}, $\gamma_{m,r}$, and $\gamma_{t,r}$, from unit-scale auxiliary variables $z \sim \operatorname{Normal}(0,1)$ and the population-scale parameters $\sigma_a$, $\sigma_b$, $\sigma_s$, $\sigma_{\gamma_m}$, and $\sigma_{\gamma_t}$, which are shared across all compounds and replicates and discussed in [Population scales](#population-scales).

To make the hierarchy concrete, consider the input-abundance effects [$a_i$]{style="color:#a3be8c"} as an example. The model contains one [$a_i$]{style="color:#a3be8c"} for every compound. Estimating all of them independently would produce unstable values for compounds with few reads. Instead, we assume that the abundance effects belong to a common population whose spread is learned from all compounds:
...
This conditional dependence is what makes the prior **hierarchical**: the compound-level parameters [$a_i$]{style="color:#a3be8c"} share the higher-level parameter $\sigma_a$. Data from all compounds inform $\sigma_a$, and the inferred $\sigma_a$ in turn controls how far each [$a_i$]{style="color:#a3be8c"} can plausibly lie from zero. This sharing produces partial pooling: uncertain abundance effects are regularised toward zero, while compounds with strong evidence can retain larger effects.

The same hierarchy is used for [$b_i$]{style="color:#88c0d0"}, [$s_i$]{style="color:#bf616a"}, $\gamma_{m,r}$, and $\gamma_{t,r}$, each with its corresponding population-scale parameter. A direct implementation of these conditional Normal priors would still require the zero-sum constraints described below.

The equations above implement this hierarchy using centred, unit-scale variables $z$. The centring and scaling operations each have a separate purpose.
