...
To assess the consistency and reproducibility of our DEL experiments, we analyze the correlations between sequencing replicates for both the matrix and target selection conditions. For each molecule, we first compute the proportion of reads it receives in a given replicate, that is, each molecule's count divided by the total sequencing counts for that replicate. By comparing these proportions across the three independent replicates, we can visualize whether the same molecules tend to be enriched to similar degrees in repeated experiments, which should appear as clustering along the diagonal in scatter plots if the replicates are highly correlated. This approach helps us detect both technical reproducibility (low technical noise) and any systematic shifts between replicates.
...
When the points cluster closely along the red $y = x$ diagonal, it shows that replicate pairs are in strong agreement: the same molecules tend to receive similar sequencing proportions in each run. This is the case for the high counts and is more visible in the matrix selection conditions. In contrast, the wide scatter seen at very low proportions is due to sparse DEL counts, where **random sampling noise is most pronounced**. This is normal and explains why low-abundance molecules often appear less reproducible.
...
[**Figure 11:** Comparing pre- and post-selection counts allows us to account for differences in the original library composition.]{.fig-caption}
...
(
    ...
    + gg.labs(x="log1p(count)", y="log1p(count)", title="Pairwise relationships: seq_load, matrix, and target")
    + gg.theme_bw()
)
```

[**Figure 12:** Pairwise relationships between `seq_load`, matrix mean, and target mean counts (log scale).]{.fig-caption}

The scatter plots of log-transformed pre-selection counts (seq_load) versus post-selection means (matrix and target conditions) reveal three features:

- **Dense mass below the diagonal:**  
  Most molecules have lower post-selection than pre-selection counts, forming a dense cluster below the $y=x$ line. This reflects the fact that most compounds do not bind the target (or the matrix), so they get washed away during the selection step. The horizontal and vertical lines within this mass are due to integer-valued counts in the post and pre-selection counts.
  

- **Points along the diagonal at high counts:**  
  Some molecules fall near the diagonal, indicating their post-selection count matches their initial abundance. These predominantly represent molecules overrepresented in the input library due to synthesis or sequencing biases, not because of selective binding. Thus, high counts can result from technical factors, and not all apparent "hits" are true binders.

- **Points above the diagonal (enriched molecules):**  
  Molecules above the $y=x$ line have higher post-selection than pre-selection counts. In the "Load vs Matrix" panel, these are likely non-specific binders to the matrix; in the "Load vs Target" panel, they are candidate true binders to the target (e.g., DDR1). More above-diagonal points in the "Load vs Target" panel is a positive signal: target selection is enriching for compounds beyond what the background alone explains.
...
1. Add a section about the Spearman correlation matrix.
2. Add a section about matrix vs target.
3. Add a section about the mean-variance relationship of the sequencing counts.
4. Add a section on modelling the counts, it shoudld take both pre-selection and matrix selections into account and compare different likelihoods.
5. Don't forget the Beta to Gamma relationship.

Then we can wrap up this section by mentioning that here we are not taking the compounds structures into account. That would be another post.
...
[**Figure 13:** Spearman correlation matrix between log-transformed pre-selection abundance (`seq_load`), matrix counts, and target counts.]{.fig-caption}



Interpretation guide:

- High `seq_load` to matrix correlation means non-specific counts are strongly driven by baseline library abundance.
- High matrix to target correlation means target counts may be dominated by background effects.
- Molecules with consistently high `target_vs_matrix` are stronger candidates for target-specific enrichment.
...
-   **Nearly identical replicates**: If the replicates are nearly identical, the variance will be very low. Example:
