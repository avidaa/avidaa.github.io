...
##### Beta-Binomial perspective on enrichment

Another useful compositional view for DEL data is the **Beta–binomial model applied to a single barcode relative to the rest of the library**. Instead of modeling the full vector of barcode proportions (Dirichlet–multinomial), we can focus on one barcode at a time.

Let $N$ be the total read depth in a sequencing run and $X_i$ the reads assigned to barcode $i$.

$$
X_i \mid p_i \sim \text{Binomial}(N, p_i)
$$

To allow **overdispersion in enrichment probabilities**, place a prior on the barcode proportion:

$$
p_i \sim \text{Beta}(a_i, b_i)
$$

Marginally, this yields a **Beta–binomial likelihood**:

$$
X_i \sim \text{BetaBinomial}(N, a_i, b_i)
$$

This model captures **extra variability in enrichment probability across barcodes**, analogous to how the Negative Binomial captures extra variability in Poisson rates.
...
When counts are small relative to $N$, the Beta–binomial model is closely related to the **Negative Binomial approximation commonly used in DEL modeling**.
:::