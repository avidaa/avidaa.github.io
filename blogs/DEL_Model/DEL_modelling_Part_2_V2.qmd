...
$$\begin{aligned}
Count_{l,i}&\sim\operatorname{NB}_2(\mu_{l,i},\phi_l),\\
Count_{m,i,r}&\sim\operatorname{NB}_2(\mu_{m,i,r},\phi_m),\\
Count_{t,i,r}&\sim\operatorname{NB}_2(\mu_{t,i,r},\phi_t).
\end{aligned}$$

$$\mathbb E(Count\mid\mu,\phi)=\mu,
\qquad
\operatorname{Var}(Count\mid\mu,\phi)
=\underbrace{\mu}_{\text{Poisson noise}}
+\underbrace{\frac{\mu^2}{\phi}}_{\text{additional variation}}.$$

The first variance term is ordinary Poisson counting noise. The second allows additional variation, beyond Poisson noise. The inverse of parameter $\phi_c$ controls the **overdispersion** for condition $c$.

This is the mean–variance convention used in the [Stan NB2 documentation](https://mc-stan.org/docs/functions-reference/unbounded_discrete_distributions.html#nbalt).

We assign one dispersion parameter, $\phi_c$, to each condition $c \in \{l, m, t\}$, shared across its compounds and replicates. It describes residual variation after the mean effects have been accounted for.

[Increasing dispersion does not systematically raise a compound's expected count; it makes observations around that expectation more variable.]{.highlight-key-point}
...
Larger $\kappa_c$ means greater spread. The limit $\kappa_c=0$ is Poisson sampling.
...
We use the **coefficient of variation (CV)** to express count variation relative to the expected count. This helps us interpret the range of variation allowed by a prior on $\kappa_c$. For example, a standard deviation of 20 reads is 20% of an expected count of 100, but only 2% of an expected count of 1,000. The CV makes this comparison by dividing the standard deviation by the mean:
...
When the expected count, $\mu$, is large enough then $1/\mu$ is small compared with $\kappa_c^2$, the CV is close to $\kappa_c$. When $\mu$ is small, the $1/\mu$ term contributes more, so the CV can be much larger than $\kappa_c$. This means that low counts can show large fluctuations relative to their mean, even when overdispersion is small.

With no overdispersion, $\kappa_c=0$, so:

$$SD = \sqrt{\mu} \quad \text{and} \quad CV = \frac{\sqrt{\mu}}{\mu} = \frac{1}{\sqrt{\mu}}$$

This is the Poisson sampling noise. For example:

- if $\mu = 1$, then $SD = 1$ read. That is 100% of the expected count.
- if $\mu = 100$, then $SD = 10$ reads. That is 10% of the expected count.

[Low expected counts can be proportionally noisy without needing any extra variation beyond ordinary counting noise, that is ordinary Poisson counting noise.]{.highlight-key-point}
:::
...
Each condition baseline receives $\alpha_c\sim\operatorname{Normal}(0,1)$.

```{python}
#| label: fig-baseline-count-interpretation
#| fig-cap: "Count-scale interpretation of the condition baseline prior. With exposure E = 100 and compound and replicate effects set to zero, the baseline expected count is 100 exp(α). Shading contains the central 95% of prior probability; the median is 100 reads. The tails continue beyond this interval. This is a prior for the baseline expected count, not a prediction interval for observed counts or the arithmetic average across compounds."
#| fig-width: 9
#| fig-height: 3.8
#| fig-dpi: 180
...
baseline_prior = lognorm(s=1, scale=baseline_exposure)
baseline_lower, baseline_median, baseline_upper = baseline_prior.ppf([0.025, 0.5, 0.975])
baseline_grid = np.linspace(0.1, 1000, 2000)
baseline_density = baseline_prior.pdf(baseline_grid)
alpha_grid = np.linspace(-3.5, 3.5, 800)
alpha_bounds = norm.ppf([0.025, 0.975])

fig, axes = new_figure(1, 2, figsize=(9, 3.8))
for ax in axes:
    style_axis(ax)
    ax.grid(False, axis="x")
    ax.set_ylim(bottom=0)

axes[0].plot(alpha_grid, norm.pdf(alpha_grid), color=COLORS["matrix"], lw=2)
axes[0].fill_between(alpha_grid, norm.pdf(alpha_grid),
                     where=(alpha_grid >= alpha_bounds[0]) & (alpha_grid <= alpha_bounds[1]),
                     color=COLORS["matrix"], alpha=0.2)
axes[0].axvline(0, color=COLORS["median"], ls="--", lw=1.3)
axes[0].set(xlabel=r"Condition baseline $\alpha_c$", ylabel="Prior density",
            xlim=(-3.5, 3.5), ylim=(0, 0.49))
panel_title(axes[0], r"Log scale: $\alpha_c \sim \mathrm{Normal}(0,1)$")
axes[0].text(0.5, 0.95, "Median 0 · Central 95%: −1.96 to 1.96",
              transform=axes[0].transAxes, ha="center", va="top",
              fontsize=0.8 * BASE_SIZE, color="#3b4252",
              bbox={"facecolor": "white", "edgecolor": "none", "pad": 3})

axes[1].plot(baseline_grid, baseline_density, color=COLORS["prior"], lw=2)
axes[1].fill_between(baseline_grid, baseline_density,
                     where=(baseline_grid >= baseline_lower) & (baseline_grid <= baseline_upper),
                     color=COLORS["prior"], alpha=0.2)
axes[1].axvline(baseline_median, color=COLORS["median"], ls="--", lw=1.3)
for bound in (baseline_lower, baseline_upper):
    axes[1].vlines(bound, 0, baseline_prior.pdf(bound),
                   color=COLORS["prior"], lw=1, alpha=0.6)
axes[1].set(xlabel="Baseline expected count (reads)", ylabel="Prior density",
            xlim=(0, 1000), ylim=(0, baseline_density.max() * 1.15))
panel_title(axes[1], r"Count scale: $\mu_{\mathrm{baseline}}=100e^{\alpha_c}$")
axes[1].text(0.96, 0.93,
              f"Exposure: {baseline_exposure} reads\n"
              f"Median: {baseline_median:.0f} reads\n"
              f"Central 95%: {baseline_lower:.0f}–{baseline_upper:.0f} reads",
              transform=axes[1].transAxes, ha="right", va="top",
              fontsize=0.85 * BASE_SIZE, linespacing=1.6, color="#3b4252",
              bbox={"facecolor": "white", "edgecolor": "none", "pad": 3})
show_figure(fig)
```


Each type of effect has its own shared scale. For example, $\sigma_a$ controls how much input abundance varies between compounds, while $\sigma_{\gamma_m}$ controls how much matrix replicate effects vary between samples.

**A small scale keeps the effects close to zero; a large scale allows greater differences.** We do not fix these scales in advance. We estimate them from the data, using the following priors to describe plausible values:
...
Each $\sigma$ controls the spread of its group's effects on the log scale. The Half-Normal prior keeps it non-negative. Its second argument sets the prior's spread: **$0.75$ is neither a fixed value of $\sigma$ nor an upper limit.** These choices allow broader variation in abundance and background, less in target-specific enrichment, and still less in replicate effects.

To see what this means for counts, start with a baseline expected count of 100 and apply one effect $x$:

$$\mu=100e^x.$$

An effect of zero leaves the expected count at 100; positive effects increase it and negative effects decrease it. The figure below shows the range of expected counts allowed by each prior. For each simulation, we draw the group's scale, draw and centre its effects so they sum to zero, and convert one effect to an expected count. This includes uncertainty about both the scale and the individual effect.

```{python}
#| label: fig-shared-scale-count-interpretation
#| fig-cap: "Prior distributions of expected counts when one effect acts on a baseline of 100 reads and all other effects are zero. Each panel uses 60,000 simulations, with 100 compounds or four replicates per simulated group. Darker shading marks the central 95% prior interval; the dashed line marks the baseline of 100. Abundance and background have the same prior specification, as do matrix and target replicate effects. The tails extend beyond the displayed range. Observed counts would have additional sampling variation."
#| fig-width: 10
#| fig-height: 3.8
#| fig-dpi: 180

scale_count_rng = np.random.default_rng(20260925)
scale_count_draws = 60_000
scale_count_specs = [
    ("Abundance / background", 0.75, 100, "#5e81ac"),
    ("Target-specific enrichment", 0.50, 100, "#bf616a"),
    ("Replicate effects", 0.25, 4, "#277F8E"),
]
scale_count_bins = np.linspace(0, 600, 121)
scale_count_centres = (scale_count_bins[:-1] + scale_count_bins[1:]) / 2
fig, axes = new_figure(1, 3, figsize=(10, 3.8), sharex=True, sharey=True)

for ax, (title, tau, group_size, color) in zip(axes, scale_count_specs):
    scales = np.abs(scale_count_rng.normal(0, tau, scale_count_draws))
    z = scale_count_rng.normal(size=(scale_count_draws, group_size))
    # A single member of each group, after imposing the zero-sum constraint.
    effect = scales * (z[:, 0] - z.mean(axis=1))
    expected_counts = 100 * np.exp(effect)
    lower, upper = np.quantile(expected_counts, [0.025, 0.975])
    frequencies, _ = np.histogram(expected_counts, bins=scale_count_bins)
    # Divide by all simulations, including draws beyond the displayed range.
    density = frequencies / (scale_count_draws * np.diff(scale_count_bins))
    central_density = np.where(
        (scale_count_centres >= lower) & (scale_count_centres <= upper), density, 0
    )
    style_axis(ax)
    ax.grid(False, axis="x")
    ax.stairs(density, scale_count_bins, fill=True, color=color, alpha=0.12)
    ax.stairs(central_density, scale_count_bins, fill=True, color=color, alpha=0.3)
    ax.stairs(density, scale_count_bins, color=color, linewidth=1.2)
    ax.axvline(100, color="#4c566a", linestyle="--", linewidth=1.2)
    ax.set(xlim=(0, 600), xticks=[0, 100, 300, 600],
           xlabel="Expected count (reads)")
    panel_title(ax, title)
    ax.text(0.97, 0.95, f"Central 95%\n{lower:.0f}–{upper:.0f} reads",
            transform=ax.transAxes, ha="right", va="top",
            fontsize=0.85 * BASE_SIZE, color="#3b4252", linespacing=1.5,
            bbox={"facecolor": "white", "edgecolor": "none", "pad": 3})
...
The narrower replicate prior keeps expected counts closer to 100, while the broader abundance and background priors allow larger departures. These are prior distributions of expected counts; they do not include the additional count variation described by the Negative Binomial likelihood.
