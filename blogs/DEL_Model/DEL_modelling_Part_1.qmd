...
import s3fs  # needed for S3 access
...
#### Two data-generation perspectives for DEL counts {#two-data-generation-perspectives-for-del-counts}
...
::: {#when-is-negative-binomial-appropriate .callout-note icon="false" collapse="true"}
## When is Negative Binomial appropriate?

Use a Negative Binomial model when the variance increases faster than the mean. In the NB2 parameterisation [Cameron and Trivedi (1986)](https://doi.org/10.1002/jae.3950010104), the mean and variance are:

$$\mathbb{E}[X_i\mid\mu_i,\phi]=\mu_i,
\qquad
\operatorname{Var}(X_i\mid\mu_i,\phi)
    =\mu_i+\frac{\mu_i^2}{\phi}.$$

Here, $\phi>0$ is an **inverse-dispersion parameter**. Smaller values of $\phi$ imply more extra-Poisson variation, whereas larger values imply less. As $\phi\rightarrow\infty$, the variance approaches the Poisson variance, $\operatorname{Var}(X_i)=\mu_i$.

The dispersion parameter is not intrinsically biological or technical. It describes residual count variation that remains after the mean model has accounted for known structure. In DEL data, that residual variation may arise from PCR amplification, sequencing, selection variability, unmodelled compound heterogeneity, or misspecification of the mean model. Interpreting $\phi$ as purely technical is therefore an additional scientific assumption, not a property of the Negative Binomial distribution.

For paired pre- and post-selection counts, there are several modelling choices:

1.  **Use one shared inverse-dispersion parameter.**

    $$X_i^\text{pre}\sim\operatorname{NB2}(\mu_i^\text{pre},\phi),
    \qquad
    X_i^\text{post}\sim\operatorname{NB2}(\mu_i^\text{post},\phi).$$

    This pools information across conditions and is more statistically efficient, but it assumes that both conditions have the same residual mean–variance relationship.

2.  **Estimate a separate parameter for each condition.**

    $$X_i^\text{pre}\sim\operatorname{NB2}(\mu_i^\text{pre},\phi_\text{pre}),
    \qquad
    X_i^\text{post}\sim\operatorname{NB2}(\mu_i^\text{post},\phi_\text{post}).$$

    This allows the conditions to have different residual variability, but introduces more parameters and requires enough information to estimate them reliably.

3.  **Use hierarchical or shared-prior dispersion parameters.**

    $$\phi_\text{pre},\phi_\text{post}\sim\text{common population distribution}.$$

    This partially pools the condition-specific estimates and provides a compromise between complete sharing and complete separation.

Because the conditions share parts of the PCR, sequencing, and library workflow, a common or partially pooled dispersion model can be a reasonable starting point. Separate parameters are justified when prior knowledge or predictive checks indicate condition-specific residual variation.
:::
...
Therefore, the PMF of the binomial distribution becomes: $$\Pr(X_i=k)=\binom{N}{k}\left(\frac{\lambda_i}{N}\right)^k\left(1-\frac{\lambda_i}{N}\right)^{N-k}
=
...
::: {style="text-align: center;"}
<img src="img/kindel_beta_shape_plot.png" alt="Effect of increasing b in a Beta(a, b) prior while keeping a = 2 fixed." style="max-width: 100%; height: auto;"/>
:::
...
::: {style="text-align: center;"}
<img src="img/kindel_beta_depth_plot.png" alt="Comparison of the example priors Beta(2, 10^3) and Beta(2, 10^6) on the success-probability scale." style="max-width: 100%; height: auto;"/>
:::
...
# Load both files
df_full = pd.read_parquet("s3://kin-del-2024/data/ddr1_1M.parquet", filesystem=fs)
df_splits = pd.read_parquet("s3://kin-del-2024/data/splits/ddr1_random.parquet", filesystem=fs)

# Filter by row position — split1 column, "train" rows
train_mask = df_splits["split1"] == "train"
df_train = df_full[train_mask.values]
...
- `target_enrichment`: Target enrichment score (a Poisson‑enrichment score comparing target vs control/matrix across replicates).
...
<!-- ![Distribution of counts by condition for the KinDEL DDR1 `split1_random` training subset.](img/kindel_eda_seq_counts.png){#fig-seq-counts} -->
...
![Cumulative distribution function (CDF) of the sequencing counts for all conditions with the 85th percentile marked.](img/kindel_ecdf_by_condition.png){#fig-ecdf-counts}
...
print(frequency_of_counts_plot)
```

![Frequency of each sequencing count value ($\leq 20$) for all conditions.](img/kindel_frequency_of_counts.png){#fig-count-frequency}
...
![Each experimental selection (both *matrix* and *target*) was performed in three independent replicates.](img/matrix_target_replicates.png){#fig-replicate-design width="75%"}
...
![Correlations between replicates for the matrix and target selection conditions.](img/Replicates_Correlations.png){#fig-replicate-scatter}
...
::: {#why-does-sampling-noise-dominate-at-low-proportions .callout-note icon="false" collapse="true"}
...
Divide both numerator and denominator by $\sqrt{p_i}$: $$\frac{\sqrt{p_i}}{\sqrt{N} \cdot p_i}
...
- **PCR amplification noise**: This multiplicative noise affects any enriched compound. [Target-enriched compounds tend to have higher absolute counts (higher $n_{\text{true}}$), so the variance scales up more dramatically.]{.highlight-key-point}
  ...
  This follows from the properties of variance: $$  \text{Var}(X)
...
![Spearman correlation matrix across matrix and target replicates using within-replicate proportions.](img/kindel_replicate_heatmap.png){#fig-replicate-heatmap}
...
![Fraction of molecules observed in 0, 1, 2, or 3 replicates for matrix and target selections.](img/kindel_replicate_support.png){#fig-replicate-support}
...
![Pairwise relationships between `seq_load`, matrix mean, and target mean counts on the log scale.](img/kindel_pre_post_scatter.png){#fig-pre-post-scatter}
...
![Spearman correlation matrix between log-transformed pre-selection abundance (`seq_load`), matrix counts, and target counts.](img/kindel_pre_post_corr.png){#fig-pre-post-corr}
...
![Relationship between mean matrix and target replicate counts on the log scale.](img/kindel_matrix_target_scatter.png){#fig-matrix-target-scatter}
...
![Crude target-over-matrix enrichment plotted against pre-selection abundance. The dashed line marks equal target and matrix signal.](img/kindel_target_over_matrix_enrichment_vs_load.png){#fig-enrichment-vs-load}
...
![Mean-variance relationship of the sequencing counts for the matrix and target selection conditions.](img/kindel_mean_variance.png){#fig-mean-variance}
